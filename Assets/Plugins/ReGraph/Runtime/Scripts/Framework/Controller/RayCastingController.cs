...
namespace Reshape.ReFramework
{
    ...
    public class RayCastingController : BaseBehaviour
    {
        ...
        public class RayReceiver
        {
            ...
            public void HandleApproveRay (string ray)
            {
                if (runner.activated) 
                    runner.TriggerRay(TriggerNode.Type.RayArrive, ray);
            }
        }
        ...
        [SpecialName]
        public bool AddReceiver (GraphRunner receiverRunner, string rayName)
        {
            bool newly = false;
            for (int i = 0; i < rays.Count; i++)
            {
                ...
                if (ray.rayName == rayName)
                {
                    ...
                    {
                        if (ray.receivers[j].runner == receiverRunner)
                        {
                    ...
                    if (!found)
                    {
                        var receiver = new RayReceiver(receiverRunner, true);
                        ray.receivers.Add(receiver);
                        receiver.HandleApproveRay(rayName);
                        newly = true;
                    }
            ...
            for (int i = 0; i < rays.Count; i++)
            {
                ...
                if (ray.type == CastType.CastFromCameraToWorld)
                {
                    ...
                    if (hit != null)
                    {
                        ...
                        PrintLog($"Hit on {theHit.transform.gameObject.name}");
        ...
        private void PrintLog (string message)
        {
            if (printLog)
            {
                ReDebug.Log("Ray Casting Controller", message);
            }
        }
    }
}