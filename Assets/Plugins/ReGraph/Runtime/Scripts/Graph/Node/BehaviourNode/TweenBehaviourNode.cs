...
namespace Reshape.ReGraph
{
    ...
    public class TweenBehaviourNode : BehaviourNode
    {
        ...
        [InfoBox("@tweenObject.GetMismatchWarningMessage()", InfoMessageType.Error, "@tweenObject.IsShowMismatchWarning()")]
        ...
        [DisableIf("ShowTweenDataWarning")]
        [InfoBox("Please save your scene so these property will be editable", InfoMessageType.Warning, "ShowTweenDataWarning", GUIAlwaysEnabled = true)]
        ...
        protected override void OnStart (GraphExecution execution, int updateId)
        {
            ...
            {
                LogWarning("Found an empty Tween Behaviour node in " + context.objectName);
            }
            else if (executionType == ExecutionType.PlayTween)
            {
                if (tweenType != TweenType.EnvLight && (tweenObject.IsEmpty || !tweenObject.IsMatchType() || tweenData == null))
                {
                    LogWarning("Found an empty Tween Behaviour node in " + context.objectName);
                }
                else
                {
                    ...
                    else if (tweenType == TweenType.EnvLight)
                    {
                        tween = tweenData.GetTween((EnvLight)null);
                        tween.SetTarget(context.transform);
                    }
            ...
            {
                if (tweenObject.IsEmpty || !tweenObject.IsMatchType() || string.IsNullOrEmpty(tweenName))
                {
                    LogWarning("Found an empty Tween Behaviour node in " + context.objectName);
                }
                else
            ...
            {
                if (tweenObject.IsEmpty || !tweenObject.IsMatchType())
                {
                    LogWarning("Found an empty Tween Behaviour node in " + context.objectName);
                }
                else
        ...
        private bool ShowTweenDataWarning ()
        {
            var graph = GetGraph();
            if (graph != null)
            {
                if (!graph.haveValidGraphId)
                    return true;
        ...
        private void CreateTweenData ()
        {
            ...
            var savePath = settings.tweenDataSaveFolder;
            if (string.IsNullOrEmpty(savePath)) return;
            if (tweenType == TweenType.Transform)
                tweenData = TweenData.CreateTweenData<TransformTween>(id, savePath);
            else if (tweenType == TweenType.Rigidbody)
                tweenData = TweenData.CreateTweenData<RigidbodyTween>(id, savePath);
            else if (tweenType == TweenType.Rigidbody2D)
                tweenData = TweenData.CreateTweenData<Rigidbody2DTween>(id, savePath);
            else if (tweenType == TweenType.RectTransform)
                tweenData = TweenData.CreateTweenData<RectTween>(id, savePath);
            else if (tweenType == TweenType.Light)
                tweenData = TweenData.CreateTweenData<LightTween>(id, savePath);
            else if (tweenType == TweenType.EnvLight)
                tweenData = TweenData.CreateTweenData<EnvLightTween>(id, savePath);
            else if (tweenType == TweenType.Camera)
                tweenData = TweenData.CreateTweenData<CameraTween>(id, savePath);
            else if (tweenType == TweenType.AudioMixer)
                tweenData = TweenData.CreateTweenData<AudioMixerTween>(id, savePath);
            else if (tweenType == TweenType.AudioSource)
                tweenData = TweenData.CreateTweenData<AudioSourceTween>(id, savePath);
            else if (tweenType == TweenType.ParticleSystem)
                tweenData = TweenData.CreateTweenData<ParticleSystemTween>(id, savePath);
            else if (tweenType == TweenType.Material)
                tweenData = TweenData.CreateTweenData<MaterialTween>(id, savePath);
            else if (tweenType == TweenType.SpriteRenderer)
                tweenData = TweenData.CreateTweenData<SpriteRendererTween>(id, savePath);
            else if (tweenType == TweenType.LineRenderer)
                tweenData = TweenData.CreateTweenData<LineRendererTween>(id, savePath);
            else if (tweenType == TweenType.TrailRenderer)
                tweenData = TweenData.CreateTweenData<TrailRendererTween>(id, savePath);
            else if (tweenType == TweenType.Graphic)
                tweenData = TweenData.CreateTweenData<GraphicTween>(id, savePath);
            else if (tweenType == TweenType.Image)
                tweenData = TweenData.CreateTweenData<ImageTween>(id, savePath);
            else if (tweenType == TweenType.Text)
                tweenData = TweenData.CreateTweenData<TextTween>(id, savePath);
            else if (tweenType == TweenType.Slider)
                tweenData = TweenData.CreateTweenData<SliderTween>(id, savePath);
            else if (tweenType == TweenType.CanvasGroup)
                tweenData = TweenData.CreateTweenData<CanvasGroupTween>(id, savePath);
            else if (tweenType == TweenType.LayoutElement)
                tweenData = TweenData.CreateTweenData<LayoutElementTween>(id, savePath);
            else if (tweenType == TweenType.ScrollRect)
                tweenData = TweenData.CreateTweenData<ScrollRectTween>(id, savePath);
            else if (tweenType == TweenType.TextMeshProGUI)
                tweenData = TweenData.CreateTweenData<TextMeshProUGUITween>(id, savePath);
            else if (tweenType == TweenType.TextMeshPro)
                tweenData = TweenData.CreateTweenData<TextMeshProTween>(id, savePath);
            MarkDirty();
        }
        ...
        {
            if (executionType == ExecutionType.PlayTween && tweenType == TweenType.EnvLight)
                return false;
            if (executionType == ExecutionType.PlayTween && tweenType != TweenType.None)
                return true;
        ...
        public override void OnUpdateGraphId (string previousGraphId, string newId)
        {
            ...
            if (tweenData.id.Equals(previousId, StringComparison.Ordinal))
            {
                ...
                var savePath = settings.tweenDataSaveFolder;
                if (string.IsNullOrEmpty(savePath)) return;
                tweenData = TweenData.CloneTweenData(GenerateId(newId), savePath, tweenData);
            }
        }
        ...
        public override void OnDelete ()
        {
            var settings = RuntimeSettings.GetSettings();
            if (settings == null) return;
            if (!settings.removeAtDeleteNode) return;
            if (tweenData == null) return;
            if (settings == null) return;
            var savePath = settings.tweenDataSaveFolder;
            if (string.IsNullOrEmpty(savePath)) return;
            var sceneFolder = Path.GetDirectoryName(savePath);
            var folderName = Path.GetFileNameWithoutExtension(savePath);
#if UNITY_EDITOR_WIN
            string dataFolder = sceneFolder + $"\\{folderName}\\";
#elif UNITY_EDITOR_OSX
            var dataFolder = sceneFolder + $"/{folderName}/";
#endif
            var fileName = id;
            var filePath = dataFolder + fileName + ".asset";
        ...
        public override void OnClone (GraphNode selectedNode)
        {
            ...
            if (node.tweenData != null)
            {
                ...
                {
                    tweenData = TweenData.CloneTweenData(id, RuntimeSettings.GetSettings().tweenDataSaveFolder, node.tweenData);
                }
                else
        ...
        {
            if (executionType == ExecutionType.PlayTween && tweenType == TweenType.EnvLight && tweenData != null)
            {
            ...
            else if (executionType == ExecutionType.PlayTween && tweenType != TweenType.None && tweenData != null && !tweenObject.IsNull && tweenObject.IsMatchType())
            {
                var tweenDataName = tweenData.GetType().ToString();
                tweenDataName = tweenDataName.Substring(20);
                return $"Play {tweenDataName.SplitCamelCase()}\n<color=#FFF600>Continue at tween end</color>";
            ...
            else if (executionType == ExecutionType.StopAnyTween && !tweenObject.IsNull && tweenObject.IsMatchType())
            {
        ...
        public override string GetNodeViewTooltip ()
        {
            ...
            if (executionType == ExecutionType.PlayTween)
                tip += "This will play a tween animation.\n\n";
            else if (executionType == ExecutionType.StopTween)
                tip += "This will stop a specific tween animation.\n\n";
            else if (executionType == ExecutionType.StopAnyTween)
                tip += "This will stop all tween animation for the specific component.\n\n";
            else
                tip += "This will execute all Tween animation related behaviour.\n\n";
