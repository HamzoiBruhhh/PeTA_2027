...
namespace Reshape.ReGraph
{
    ...
    public class NavMeshBehaviourNode : BehaviourNode
    {
        ...
        [SerializeField]
        [ShowIf("@executionType == ExecutionType.AgentWalk || executionType == ExecutionType.AgentChase")]
        ...
        protected override void OnStart (GraphExecution execution, int updateId)
        {
            ...
            {
                LogWarning("Found an empty NavMesh Behaviour node in " + context.objectName);
            }
            ...
            {
                if (((NavMeshAgent)agent).TryGetComponent(out AgentChaseController chase))
                    chase.Terminate();
            }
        ...
        public override string GetNodeViewDescription ()
        {
            if (executionType != ExecutionType.None && !agent.IsNull && agent.IsMatchType())
            {
                ...
                if (executionType == ExecutionType.AgentGiveUp)
                {
                    return agent.name + " give up chase";
                }
        ...
        {
            return "This will provide several controls to a specific NavMeshAgent.\n\n" + base.GetNodeViewTooltip();
        }
#endif
    }
}