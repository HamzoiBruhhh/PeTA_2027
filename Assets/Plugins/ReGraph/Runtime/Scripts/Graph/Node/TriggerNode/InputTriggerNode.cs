...
namespace Reshape.ReGraph
{
    ...
    public class InputTriggerNode : TriggerNode
    {
        ...
        {
#if ENABLE_INPUT_SYSTEM
            if (inputAction && triggerType != Type.None)
            {
                ...
                {
                    LogWarning("Found an empty Input Trigger node in " + context.objectName);
                }
            }
            else
            {
        ...
        public override string GetNodeViewTooltip ()
        {
            ...
            if (triggerType == Type.InputPress)
                tip += "This will get trigger when the configured input have fire.\n\n";
            else if (triggerType == Type.InputRelease)
                tip += "This will get trigger when the configured input have release.\n\n";
            else
