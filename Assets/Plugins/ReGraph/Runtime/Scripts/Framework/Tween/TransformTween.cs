...
namespace Reshape.ReFramework
{
    public class TransformTween : TweenData
    {
        ...
        public class VectorThree 
        {
            ...
            public static implicit operator Vector3 (VectorThree v3)
            {
                if (v3 == null)
                    return default;
                var v = Vector3.zero;
            ...
            public static Vector3[] FormArray (VectorThree[] v3Array)
            {
                var arr = new Vector3[v3Array.Length];
                for (var i = 0; i < v3Array.Length; i++)
                    arr[i] = v3Array[i];
                return arr;
            }
        }
        ...
        public override Tween GetTween (Transform transform)
        {
            switch (command)
            {
                ...
                case TransformCommand.Path:
                    return transform.DOPath(VectorThree.FormArray(waypoints), duration, pathType, pathMode);
                case TransformCommand.LocalPath:
                    return transform.DOLocalPath(VectorThree.FormArray(waypoints), duration, pathType, pathMode);
