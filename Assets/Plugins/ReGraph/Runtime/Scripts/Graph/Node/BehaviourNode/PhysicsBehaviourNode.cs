...
namespace Reshape.ReGraph
{
    ...
    public class PhysicsBehaviourNode : BehaviourNode
    {
        ...
        [InfoBox("@rigidbody.GetMismatchWarningMessage()", InfoMessageType.Error, "@rigidbody.IsShowMismatchWarning()")]
        private SceneObjectProperty rigidbody = new SceneObjectProperty(SceneObject.ObjectType.Rigidbody);

        protected override void OnStart (GraphExecution execution, int updateId)
        {
            if (rigidbody.IsEmpty || !rigidbody.IsMatchType() || executionType == ExecutionType.None)
            {
                LogWarning("Found an empty Physics Behaviour node in " + context.objectName);
            }
        ...
        {
            if (!rigidbody.IsNull && rigidbody.IsMatchType())
            {
        ...
        {
            return "This will provide several controls to a specific Rigidbody.\n\n" + base.GetNodeViewTooltip();
        }
#endif
    }
}