...
{
    [HideMonoScript]
    public class FloatTextController : VisualEffectController
    {
        public const int TYPE_FACE_CAMERA = 1;
        public const int TYPE_FOLLOW_CAMERA = 2;
        private const string POOL_TYPE = "FloatText";
        ...
        public static FloatTextController Spawn (string msg, Transform loc, string name = "", Transform parent = null, float size = -1, Color color = default, int type = 0, Camera cam = null,
            Transform followTrans = null, GameObject effectPrefab = null)
        {
            var vfx = effectPrefab;
            if (!vfx)
                vfx = GraphManager.instance.runtimeSettings.floatText;
            if (vfx)
            {
                var text = ReMonoBehaviour.TakeFromPool(POOL_TYPE, vfx, loc, true, parent);
                if (text)
                {
                    text.name = string.IsNullOrWhiteSpace(name) ? GraphManager.instance.runtimeSettings.floatText.name : name;
                    if (text.TryGetComponent<FloatTextController>(out var controller))
                    {
